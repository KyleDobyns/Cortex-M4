...
void *_ralloc(int size, int left, int right) {
    ...
    // Check if the size can fit within half the current space
    if (size <= ((right - left + mcb_ent_sz) / 2) * 16) {
        // Recursively allocate space in the left half
        void* new_alloc_addr = _ralloc(size, left, left + ((right - left + mcb_ent_sz) / 2) - mcb_ent_sz);
        if (new_alloc_addr == 0) {
            // If left half fails, allocate space in the right half
            return _ralloc(size, left + ((right - left + mcb_ent_sz) / 2), right);
        }
        // Check if the midpoint MCB is free
        if ((array[m2a(left + ((right - left + mcb_ent_sz) / 2))] & 0x01) == 0) {
            // Mark the midpoint MCB with the allocated size
            *(short *)&array[m2a(left + ((right - left + mcb_ent_sz) / 2))] = ((right - left + mcb_ent_sz) / 2) * 16;
        }
        return new_alloc_addr;
    } else {
        // Check if the current MCB is already allocated
        if ((array[m2a(left)] & 0x01) != 0) {
            return 0;
        } else {
            // Check if the size of the current MCB is sufficient
            if (*(short *)&array[m2a(left)] < (right - left + mcb_ent_sz) * 16) {
                return 0;
            }
            // Mark the current MCB as allocated
            *(short *)&array[m2a(left)] = (right - left + mcb_ent_sz) * 16 | 0x01;
...
int _rfree(int mcb_addr) {
    // Read the contents of the memory control block at the given address
    short mcb_contents = *(short *)&array[m2a(mcb_addr)];
    
    // Calculate the index and offset for the MCB
    int block_index = mcb_addr - mcb_top;
    int block_offset = (mcb_contents /= 16);
    int block_size = (mcb_contents *= 16);
    
    // Clear the used bit of the MCB
    *(short *)&array[m2a(mcb_addr)] = mcb_contents;
    
    // Check if the block is on an even index, indicating it's the left buddy
    if ((block_index / block_offset) % 2 == 0) {
        // The right buddy is within bounds
        if (mcb_addr + block_offset >= mcb_bot) {
            return 0;
        } else {
            // Read the contents of the buddy MCB
            short buddy_contents = *(short *)&array[m2a(mcb_addr + block_offset)];
            // Check if the buddy block is free
            if ((buddy_contents & 0x0001) == 0) {
                buddy_contents = (buddy_contents / 32) * 32;
                // Check if the buddy size matches the current block size
                if (buddy_contents == block_size) {
                    // Merge the buddy with the current block
                    *(short *)&array[m2a(mcb_addr + block_offset)] = 0;
                    block_size *= 2;
                    *(short *)&array[m2a(mcb_addr)] = block_size;
                    return _rfree(mcb_addr);
                }
            }
        }
    } else {
        // The left buddy is within bounds
        if (mcb_addr - block_offset < mcb_top) {
            return 0;
        } else {
            // Read the contents of the buddy MCB
            short buddy_contents = *(short *)&array[m2a(mcb_addr - block_offset)];
            ...
            if ((buddy_contents & 0x0001) == 0) {
                ...
                if (buddy_contents == block_size) {
                    ...
                    *(short *)&array[m2a(mcb_addr)] = 0;
                    block_size *= 2;
                    *(short *)&array[m2a(mcb_addr - block_offset)] = block_size;
                    return _rfree(mcb_addr - block_offset);
                }
            }
        }
    }

    // Return the address of the MCB that was successfully freed
    return mcb_addr;
}
...
// gcc heap.c driver_cpg.c -o a.out
// ./a.out